...
enum I2cIsrCmd { readMst, writeMst, readSlv, writeSlv, waitSI };
...
// "volatile" has been omitted since ISR and thread do not run simultaneously,
// hopefully ok
...
// little helpers cloned from official i2c api
static inline void i2c_conclr(i2c_t* obj, int start, int stop, int interrupt,
                              int acknowledge) {
    I2C_CONCLR(obj) =
        (start << 5) | (stop << 4) | (interrupt << 3) | (acknowledge << 2);
}
static inline void i2c_conset(i2c_t* obj, int start, int stop, int interrupt,
                              int acknowledge) {
    I2C_CONSET(obj) =
        (start << 5) | (stop << 4) | (interrupt << 3) | (acknowledge << 2);
}
static inline void i2c_clear_SI(i2c_t* obj) { i2c_conclr(obj, 0, 0, 1, 0); }
...
static void i2cRtos_isr(uint32_t ch) {
    ...
    if (tr->cmd == waitSI) {
        // just waiting for an interrupt after a byte read/write or slave
        // receive call
...
#pragma GCC diagnostic ignored "-Wswitch"
    switch (tr->cmd) {
            ...
            switch (stat) {
                ...
                case 0x40:  // SLA+R has been transmitted; ACK has been
                    ...
                    if (tr->cnt != tr->len - 1)
                        ...
                        i2c_conclr(tr->obj, 0, 0, 0,
                                   1);  // do not ack the last byte read
                    stay = 1;
                    break;
        ...
        case readSlv:
            ...
            // SLV+W???
            // 70: General Call address (0x00) has been received; ACK has been
            // returned.
            break;
...
#if defined(TARGET_LPC1768) || defined(TARGET_LPC2368)
static void i2cRtos_isr_ch0() { i2cRtos_isr(0); }
#endif

static void i2cRtos_isr_ch1() { i2cRtos_isr(1); }
...
static inline void i2cRtos_wait_and_see(i2c_t* obj, int ch,
                                        uint32_t tmOut)  // evillive
{
    ...
    if (osSemaphoreWait(iis->sem, tmOut) != 1)
        NVIC_DisableIRQ(iis->irq);  // time out => diable the IRQ
}
...
int i2cRtos_read(i2c_t* obj, int address, char* data, int length, int stop) {
    ...
    tr->rData = data;
    I2C_DAT(obj) =
        address | 0x01;  // initiate address+R write and enter isr mode
...
int i2cRtos_write(i2c_t* obj, int address, const char* data, int length,
    ...
    tr->wData = data;
    I2C_DAT(obj) =
        address & 0xfe;  // initiate address+W write and enter isr mode
    i2cRtos_wait_and_see(obj, ch, 2 + (length >> 2));  // timeout (2+len/4)ms
    // i2c_clear_SI(obj); // ... why? Also in official lib ... I guess this is
    // the "write instead of start" bug
    status = tr->stat;
    // gpio_write(&gpio[1], 1);
...
void i2cRtos_init(i2c_t* obj, PinName sda, PinName scl) {
    ...
    switch (ch) {
        case 0:
            ...
            isrIrqSem[ch].sem =
                osSemaphoreCreate(osSemaphore(i2cIsrDrvSem_ch0), 1);
            break;
        case 1:
            isrIrqSem[ch].irq = I2C2_IRQn;
            NVIC_SetVector(I2C2_IRQn, (uint32_t)i2cRtos_isr_ch1);
            isrIrqSem[ch].sem =
                osSemaphoreCreate(osSemaphore(i2cIsrDrvSem_ch1), 1);
            break;
    }

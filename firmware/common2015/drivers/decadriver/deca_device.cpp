...
int dw1000_api::dwt_initialise(uint16 config) {
    uint16 otp_addr = 0;
    uint32 ldo_tune = 0;
    ...
    _dwt_enableclocks(FORCE_SYS_XTI);  // NOTE: set system clock to XTI - this
                                       // is necessary to make sure the values
    ...
    // Load LDO tune from OTP and kick it if there is a value actually
    // programmed.
    ldo_tune = _dwt_otpread(LDOTUNE_ADDRESS);
    if ((ldo_tune & 0xFF) != 0) {
        // Kick LDO tune
        dwt_write8bitoffsetreg(OTP_IF_ID, OTP_SF,
                               OTP_SF_LDO_KICK);  // Set load LDE kick bit
    ...
    if (!dw1000local.init_xtrim)  // A value of 0 means that the crystal has not
                                  // been trimmed
    {
 ...
 */
uint8 dw1000_api::dwt_otprevision(void) { return dw1000local.otprev; }
 ...
 * no return value
 */
void dw1000_api::dwt_setfinegraintxseq(int enable) {
    if (enable) {
 ...
 * handy to monitor DW1000's activity.
 *
 ...
 * no return value
 */
void dw1000_api::dwt_setlnapamode(int lna, int pa) {
 ...
 * no return value
 */
void dw1000_api::dwt_setgpiodirection(uint32 gpioNum, uint32 direction) {
 ...
 * no return value
 */
void dw1000_api::dwt_setgpiovalue(uint32 gpioNum, uint32 value) {
 ...
 */
uint32 dw1000_api::dwt_getpartid(void) { return dw1000local.partID; }
 ...
 */
uint32 dw1000_api::dwt_getlotid(void) { return dw1000local.lotID; }
 ...
 */
uint32 dw1000_api::dwt_readdevid(void) {
 ...
 * no return value
 */
void dw1000_api::dwt_configuretxrf(dwt_txconfig_t* config) {
 ...
 * no return value
 */
void dw1000_api::dwt_configure(dwt_config_t* config) {
    ...
    assert(((config->prf == DWT_PRF_64M) && (config->txCode >= 9) &&
            (config->txCode <= 24)) ||
           ((config->prf == DWT_PRF_16M) && (config->txCode >= 1) &&
            (config->txCode <= 8)));
    assert(((config->prf == DWT_PRF_64M) && (config->rxCode >= 9) &&
            (config->rxCode <= 24)) ||
           ((config->prf == DWT_PRF_16M) && (config->rxCode >= 1) &&
            (config->rxCode <= 8)));
    ...
    dw1000local.sysCFGreg |=
        (SYS_CFG_PHR_MODE_11 & (config->phrMode << SYS_CFG_PHR_MODE_SHFT));
    ...
    regval =
        ...
        (CHAN_CTRL_DWSFD &
         (config->nsSFD << CHAN_CTRL_DWSFD_SHIFT)) |  // Use DW nsSFD
    ...
    dw1000local.txFCTRL =
        ((config->txPreambLength | config->prf) << TX_FCTRL_TXPRF_SHFT) |
    ...
    // SYS_CTRL write below works around this issue, by simultaneously
    // initiating and aborting a transmission, which correctly initialises the
    // SFD
    // after its configuration or reconfiguration.
    // This issue is not documented at the time of writing this code. It should
    // be in next release of DW1000 User Manual (v2.09, from July 2016).
    dwt_write8bitoffsetreg(
        SYS_CTRL_ID, SYS_CTRL_OFFSET,
        SYS_CTRL_TXSTRT |
            SYS_CTRL_TRXOFF);  // Request TX start and TRX off at the same time
}  // end dwt_configure()
 ...
 * no return value
 */
void dw1000_api::dwt_setrxantennadelay(uint16 rxDelay) {
 ...
 * no return value
 */
void dw1000_api::dwt_settxantennadelay(uint16 txDelay) {
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dw1000_api::dwt_writetxdata(uint16 txFrameLength, uint8* txFrameBytes,
 ...
 * no return value
 */
void dw1000_api::dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset,
                                  int ranging) {
#ifdef DWT_API_ERROR_CHECK
    assert((dw1000local.longFrames && (txFrameLength <= 1023)) ||
           (txFrameLength <= 127));
 ...
 * no return value
 */
void dw1000_api::dwt_readrxdata(uint8* buffer, uint16 length,
 ...
 * no return value
 */
void dw1000_api::dwt_readaccdata(uint8* buffer, uint16 len, uint16 accOffset) {
 ...
 * no return value
 */
void dw1000_api::dwt_readdiagnostics(dwt_rxdiag_t* diagnostics) {
    ...
    diagnostics->maxNoise =
        dwt_read16bitoffsetreg(LDE_IF_ID, LDE_THRESH_OFFSET);
    ...
    diagnostics->firstPathAmp1 =
        dwt_read16bitoffsetreg(RX_TIME_ID, RX_TIME_FP_AMPL1_OFFSET);
 ...
 * no return value
 */
void dw1000_api::dwt_readtxtimestamp(uint8* timestamp) {
    dwt_readfromdevice(TX_TIME_ID, TX_TIME_TX_STAMP_OFFSET,
 ...
 */
uint32 dw1000_api::dwt_readtxtimestamphi32(void) {
    return dwt_read32bitoffsetreg(
        TX_TIME_ID, 1);  // Offset is 1 to get the 4 upper bytes out of 5
}
 ...
 */
uint32 dw1000_api::dwt_readtxtimestamplo32(void) {
    return dwt_read32bitreg(TX_TIME_ID);  // Read TX TIME as a 32-bit register
 ...
 * no return value
 */
void dw1000_api::dwt_readrxtimestamp(uint8* timestamp) {
    dwt_readfromdevice(RX_TIME_ID, RX_TIME_RX_STAMP_OFFSET,
 ...
 */
uint32 dw1000_api::dwt_readrxtimestamphi32(void) {
    return dwt_read32bitoffsetreg(
        RX_TIME_ID, 1);  // Offset is 1 to get the 4 upper bytes out of 5
}
 ...
 */
uint32 dw1000_api::dwt_readrxtimestamplo32(void) {
    return dwt_read32bitreg(RX_TIME_ID);  // Read RX TIME as a 32-bit register
                                          // to get the 4 lower bytes out of 5
}
 ...
 */
uint32 dw1000_api::dwt_readsystimestamphi32(void) {
    return dwt_read32bitoffsetreg(
        SYS_TIME_ID, 1);  // Offset is 1 to get the 4 upper bytes out of 5
}
 ...
 * no return value
 */
void dw1000_api::dwt_readsystime(uint8* timestamp) {
 ...
 * no return value
 */
void dw1000_api::dwt_writetodevice(uint16 recordNumber, uint16 index,
                                   uint32 length, const uint8* buffer) {
    ...
    if (index == 0)  // For index of 0, no sub-index is required
    {
        header[cnt++] = 0x80 | recordNumber;  // Bit-7 is WRITE operation, bit-6
                                              // zero=NO sub-addressing, bits
                                              // 5-0 is reg file id
    } else {
#ifdef DWT_API_ERROR_CHECK
        assert((index <= 0x7FFF) &&
               ((index + length) <= 0x7FFF));  // Index and sub-addressable area
                                               // are limited to 15-bits.
#endif
        header[cnt++] = 0xC0 | recordNumber;  // Bit-7 is WRITE operation, bit-6
                                              // one=sub-address follows, bits
                                              // 5-0 is reg file id
        ...
        {
            header[cnt++] = (uint8)
                index;  // Bit-7 zero means no extension, bits 6-0 is index.
        } else {
            header[cnt++] = 0x80 | (uint8)(index);  // Bit-7 one means extended
                                                    // index, bits 6-0 is low
                                                    // seven bits of index.
            header[cnt++] =
 ...
 * no return value
 */
void dw1000_api::dwt_readfromdevice(uint16 recordNumber, uint16 index,
    ...
    // Write message header selecting READ operation and addresses as
    // appropriate (this is one to three bytes long)
    if (index == 0)  // For index of 0, no sub-index is required
    {
        header[cnt++] = (uint8)recordNumber;  // Bit-7 zero is READ operation,
                                              // bit-6 zero=NO sub-addressing,
...
#endif
        header[cnt++] =
            (uint8)(0x40 | recordNumber);  // Bit-7 zero is READ operation,
                                           // bit-6 one=sub-address follows,
                                           // bits 5-0 is reg file id
 ...
 */
uint32 dw1000_api::dwt_read32bitoffsetreg(int regFileID, int regOffset) {
    uint32 regval = 0;
    int j;
    uint8 buffer[4];
 ...
 */
uint16 dw1000_api::dwt_read16bitoffsetreg(int regFileID, int regOffset) {
    uint16 regval = 0;
    uint8 buffer[2];
 ...
 */
uint8 dw1000_api::dwt_read8bitoffsetreg(int regFileID, int regOffset) {
    uint8 regval;
 ...
 * no return value
 */
void dw1000_api::dwt_write8bitoffsetreg(int regFileID, int regOffset,
                                        uint8 regval) {
 ...
 * no return value
 */
void dw1000_api::dwt_write16bitoffsetreg(int regFileID, int regOffset,
                                         uint16 regval) {
    uint8 buffer[2];
 ...
 * no return value
 */
void dw1000_api::dwt_write32bitoffsetreg(int regFileID, int regOffset,
                                         uint32 regval) {
    int j;
    uint8 buffer[4];
 ...
 * no return value
 */
void dw1000_api::dwt_enableframefilter(uint16 enable) {
    uint32 sysconfig =
 ...
 * no return value
 */
void dw1000_api::dwt_setpanid(uint16 panID) {
 ...
 * no return value
 */
void dw1000_api::dwt_setaddress16(uint16 shortAddress) {
 ...
 * no return value
 */
void dw1000_api::dwt_seteui(uint8* eui64) {
 ...
 * no return value
 */
void dw1000_api::dwt_geteui(uint8* eui64) {
 ...
 * no return value
 */
void dw1000_api::dwt_otpread(uint32 address, uint32* array, uint8 length) {
    int i;

    _dwt_enableclocks(FORCE_SYS_XTI);  // NOTE: Set system clock to XTAL - this
                                       // is necessary to make sure the values
                                       // read by _dwt_otpread are reliable
 ...
 */
uint32 dw1000_api::_dwt_otpread(uint32 address) {
    uint32 ret_data;
    ...
    dwt_write8bitoffsetreg(OTP_IF_ID, OTP_CTRL,
                           OTP_CTRL_OTPREAD | OTP_CTRL_OTPRDEN);
    dwt_write8bitoffsetreg(
        OTP_IF_ID, OTP_CTRL,
        0x00);  // OTPREAD is self clearing but OTPRDEN is not
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32 dw1000_api::_dwt_otpsetmrregs(int mode) {
    uint8 rd_buf[4];
    uint8 wr_buf[4];
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32 dw1000_api::_dwt_otpprogword32(uint32 data, uint16 address) {
    uint8 rd_buf[1];
    uint8 wr_buf[4];
    uint8 otp_done;
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32 dw1000_api::dwt_otpwriteandverify(uint32 value, uint16 address) {
    ...
    // Even if the above does not exit before retry reaches 5, the programming
    // has probably been successful
    ...
    _dwt_otpsetmrregs(0);  // Setting OTP mode register for low RM read -
                           // resetting the device would be alternative
 ...
 * no return value
 */
void dw1000_api::_dwt_aonconfigupload(void) {
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, AON_CTRL_UPL_CFG);
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET,
                           0x00);  // Clear the register
}
 ...
 * no return value
 */
void dw1000_api::_dwt_aonarrayupload(void) {
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET,
                           0x00);  // Clear the register
 ...
 * no return value
 */
void dw1000_api::dwt_entersleep(void) {
 ...
 * no return value
 */
void dw1000_api::dwt_configuresleepcnt(uint16 sleepcnt) {
    ...
    dwt_write8bitoffsetreg(AON_ID, AON_CFG0_OFFSET,
                           0x00);  // NB: this write change the default
                                   // LPCLKDIVA value which is not used anyway.
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, 0x00);
    ...
    // Set new value
    dwt_write16bitoffsetreg(AON_ID, AON_CFG0_OFFSET + AON_CFG0_SLEEP_TIM_OFFSET,
                            sleepcnt);
    _dwt_aonconfigupload();
 ...
 */
uint16 dw1000_api::dwt_calibratesleepcnt(void) {
    uint16 result;
    ...
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET,
                           AON_CTRL_DCA_ENAB | AON_CTRL_DCA_READ);
 ...
 * no return value
 */
void dw1000_api::dwt_configuresleep(uint16 mode, uint8 wake) {
 ...
 * no return value
 */
void dw1000_api::dwt_entersleepaftertx(int enable) {
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dw1000_api::dwt_spicswakeup(uint8* buff, uint16 length) {
    if (dwt_readdevid() !=
        DWT_DEVICE_ID)  // Device was in deep sleep (the first read fails)
    {
        // Need to keep chip select line low for at least 500us
        dwt_readfromdevice(0x0, 0x0, length, buff);  // Do a long read to wake
                                                     // up the chip (hold the
                                                     // chip select low)

        // Need 5ms for XTAL to start and stabilise (could wait for PLL lock IRQ
        // status bit !!!)
        // NOTE: Polling of the STATUS register is not possible unless frequency
        // is < 3MHz
        deca_sleep(5);
    } else {
        return DWT_SUCCESS;
 ...
 * no return value
 */
void dw1000_api::_dwt_configlde(int prfIndex) {
    dwt_write8bitoffsetreg(LDE_IF_ID, LDE_CFG1_OFFSET,
                           LDE_PARAM1);  // 8-bit configuration register
 ...
 * no return value
 */
void dw1000_api::_dwt_loaducodefromrom(void) {
    // Set up clocks
 ...
 * no return value
 */
void dw1000_api::dwt_loadopsettabfromotp(uint8 ops_sel) {
    uint16 reg =
        ((ops_sel << OTP_SF_OPS_SEL_SHFT) & OTP_SF_OPS_SEL_MASK) |
 ...
 * no return value
 */
void dw1000_api::dwt_setsmarttxpower(int enable) {
 ...
 * no return value
 */
void dw1000_api::dwt_enableautoack(uint8 responseDelayTime) {
    // Set auto ACK reply delay
    dwt_write8bitoffsetreg(ACK_RESP_T_ID, ACK_RESP_T_ACK_TIM_OFFSET,
                           responseDelayTime);  // In symbols
    // Enable auto ACK
 ...
 * no return value
 */
void dw1000_api::dwt_setdblrxbuffmode(int enable) {
    if (enable) {
 ...
 * no return value
 */
void dw1000_api::dwt_setrxaftertxdelay(uint32 rxDelayTime) {
    uint32 val =
        dwt_read32bitreg(ACK_RESP_T_ID);  // Read ACK_RESP_T_ID register

    val &= ~(ACK_RESP_T_W4R_TIM_MASK);  // Clear the timer (19:0)

    val |= (rxDelayTime & ACK_RESP_T_W4R_TIM_MASK);  // In UWB microseconds
                                                     // (e.g. turn the receiver
                                                     // on 20uus after TX)
 ...
 * no return value
 */
void dw1000_api::dwt_setcallbacks(dwt_cb_t cbTxDone, dwt_cb_t cbRxOk,
 ...
 */
uint8 dw1000_api::dwt_checkirq(void) {
    return (dwt_read8bitoffsetreg(SYS_STATUS_ID, SYS_STATUS_OFFSET) &
            SYS_STATUS_IRQS);  // Reading the lower byte only is enough for this
                               // operation
}
 ...
 * @fn dwt_isr()
 *
 * @brief This is the DW1000's general Interrupt Service Routine. It will
 ...
 * no return value
 */
void dw1000_api::dwt_isr(void) {
    uint32 status = dw1000local.cbData.status =
    ...
    if (status & SYS_STATUS_RXFCG) {
        ...
        dwt_readfromdevice(RX_BUFFER_ID, 0, FCTRL_LEN_MAX,
                           dw1000local.cbData.fctrl);

        // Because of a previous frame not being received properly, AAT bit can
        // be set upon the proper reception of a frame not requesting for
        // acknowledgement (ACK frame is not actually sent though). If the AAT
        // bit is set, check ACK request bit in frame control to confirm (this
        // implementation works only for IEEE802.15.4-2011 compliant frames).
        // This issue is not documented at the time of writing this code. It
        // should be in next release of DW1000 User Manual (v2.09, from July
        // 2016).
        if ((status & SYS_STATUS_AAT) &&
            ((dw1000local.cbData.fctrl[0] & FCTRL_ACK_REQ_MASK) == 0)) {
            dwt_write32bitreg(
                SYS_STATUS_ID,
                SYS_STATUS_AAT);  // Clear AAT status bit in register
            dw1000local.cbData.status &= ~SYS_STATUS_AAT;  // Clear AAT status
                                                           // bit in callback
                                                           // data register copy
            dw1000local.wait4resp = 0;
        }
        ...
        if (dw1000local.cbRxOk != NULL) {
            (this->*dw1000local.cbRxOk)(&dw1000local.cbData);
        }
    ...
    if (status & SYS_STATUS_TXFRS) {
        dwt_write32bitreg(SYS_STATUS_ID,
                          SYS_STATUS_ALL_TX);  // Clear TX event bits

        // In the case where this TXFRS interrupt is due to the automatic
        // transmission of an ACK solicited by a response (with ACK request bit
        // set)
        // that we receive through using wait4resp to a previous TX (and
        // assuming that the IRQ processing of that TX has already been
        // handled), then
        // we need to handle the IC issue which turns on the RX again in this
        // situation (i.e. because it is wrongly applying the wait4resp after
        // the
        // ACK TX).
        // See section "Transmit and automatically wait for response" in DW1000
        // User Manual
        if ((status & SYS_STATUS_AAT) && dw1000local.wait4resp) {
            dwt_forcetrxoff();  // Turn the RX off
            dwt_rxreset();      // Reset in case we were late and a frame was
                                // already being received
        }

        // Call the corresponding callback if present
        if (dw1000local.cbTxDone != NULL) {
            (this->*dw1000local.cbTxDone)(&dw1000local.cbData);
        }
    }
    ...
    if (status & SYS_STATUS_ALL_RX_TO) {
        dwt_write32bitreg(SYS_STATUS_ID,
                          SYS_STATUS_RXRFTO);  // Clear RX timeout event bits

        dw1000local.wait4resp = 0;

        // Because of an issue with receiver restart after error conditions, an
        // RX reset must be applied after any error or timeout event to ensure
        ...
        if (dw1000local.cbRxTo != NULL) {
            (this->*dw1000local.cbRxTo)(&dw1000local.cbData);
        }
    }
    ...
    if (status & (SYS_STATUS_ALL_RX_ERR | 0x00002000)) {
        ...
        if (dw1000local.cbRxErr != NULL) {
            (this->*dw1000local.cbRxErr)(&dw1000local.cbData);
        }
    }
}
 ...
 * @fn dwt_isr_lplisten()
 *
 * @brief This is the DW1000's Interrupt Service Routine to use when low-power
 * listening scheme is implemented. It will
 *        only process/report the RXFCG event (through cbRxOk callback).
 *        It clears RXFCG interrupt and reads received frame information and
 * frame control before calling the callback.
 *
 *        /!\ This version of the ISR is designed for single buffering case
 * only!
 *
 * input parameters
 *
 ...
 * no return value
 */
void dw1000_api::dwt_lowpowerlistenisr(void) {
    uint32 status = dw1000local.cbData.status =
        dwt_read32bitreg(SYS_STATUS_ID);  // Read status register low 32bits
    uint16 finfo16;
    uint16 len;

    // The only interrupt handled when in low-power listening mode is RX good
    // frame so proceed directly to the handling of the received frame.

    // Deactivate low-power listening before clearing the interrupt. If not, the
    // DW1000 will go back to sleep as soon as the interrupt is cleared.
    ...
    // Because of a previous frame not being received properly, AAT bit can be
    // set upon the proper reception of a frame not requesting for
    // acknowledgement (ACK frame is not actually sent though). If the AAT bit
    // is set, check ACK request bit in frame control to confirm (this
    // implementation works only for IEEE802.15.4-2011 compliant frames).
    // This issue is not documented at the time of writing this code. It should
    // be in next release of DW1000 User Manual (v2.09, from July 2016).
    if ((status & SYS_STATUS_AAT) &&
        ...
        dwt_write32bitreg(SYS_STATUS_ID,
                          SYS_STATUS_AAT);  // Clear AAT status bit in register
        dw1000local.cbData.status &= ~SYS_STATUS_AAT;  // Clear AAT status bit
                                                       // in callback data
                                                       // register copy
        dw1000local.wait4resp = 0;
    }
 ...
 * no return value
 */
void dw1000_api::dwt_setleds(uint8 mode) {
    uint32 reg;
 ...
 * no return value
 */
void dw1000_api::_dwt_enableclocks(int clocks) {
    uint8 reg[2];
 ...
 * no return value
 */
void dw1000_api::_dwt_disablesequencing(
    void)  // Disable sequencing and go to state "INIT"
{
    _dwt_enableclocks(FORCE_SYS_XTI);  // Set system clock to XTI

    dwt_write16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET,
                            PMSC_CTRL1_PKTSEQ_DISABLE);  // Disable PMSC ctrl of
                                                         // RF and RX clk blocks
}
 ...
 * no return value
 */
void dw1000_api::dwt_setdelayedtrxtime(uint32 starttime) {
    dwt_write32bitoffsetreg(DX_TIME_ID, 1, starttime);  // Write at offset 1 as
                                                        // the lower 9 bits of
                                                        // this register are
                                                        // ignored
 ...
 */
int dw1000_api::dwt_starttx(uint8 mode) {
    ...
    if (mode & DWT_START_TX_DELAYED) {
        ...
        dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, temp);
        checkTxOK = dwt_read16bitoffsetreg(
            SYS_STATUS_ID,
                ...
                // individual blocks).
        {
            ...
            // I am taking DSHP set to Indicate that the TXDLYS was set too late
            // for the specified DX_TIME.
            // Remedial Action - (a) cancel delayed send
            temp = (uint8)
                SYS_CTRL_TRXOFF;  // This assumes the bit is in the lowest byte
            dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, temp);
            // Note event Delayed TX Time too Late
            // Could fall through to start a normal send (below) just sending
            // late.....
            // ... instead return and assume return value of 1 will be used to
 ...
 * no return value
 */
void dw1000_api::dwt_forcetrxoff(void) {
    ...
    // modify write cycle
    // We can disable the radio, but before the status is cleared an interrupt
    // can be set (e.g. the
    ...
    dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET,
                           (uint8)SYS_CTRL_TRXOFF);  // Disable the radio

    // Forcing Transceiver off - so we do not want to see any new events that
    // may have happened
    dwt_write32bitreg(SYS_STATUS_ID,
                      (SYS_STATUS_ALL_TX | SYS_STATUS_ALL_RX_ERR |
                       SYS_STATUS_ALL_RX_TO | SYS_STATUS_ALL_RX_GOOD));
 ...
 * no return value
 */
void dw1000_api::dwt_syncrxbufptrs(void) {
    uint8 buff;
    // Need to make sure that the host/IC buffer pointers are aligned before
    // starting RX
    buff = dwt_read8bitoffsetreg(
        SYS_STATUS_ID,
         ...
         << 1))  // Host Side Receive Buffer Pointer
    {
        dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET,
                               0x01);  // We need to swap RX buffer status reg
 ...
 * 128/125 �s (~1 �s). Max value is 255.
 *
 * output parameters
 *
 * no return value
 */
void dw1000_api::dwt_setsniffmode(int enable, uint8 timeOn, uint8 timeOff) {
    uint32 pmsc_reg;
    if (enable) {
        /* Configure ON/OFF times and enable PLL2 on/off sequencing by SNIFF
         * mode. */
 ...
 * SLEEP state but wakes periodically, (after
 * this "long sleep"), for a very short time to sample the air for a preamble
 * sequence. This preamble sampling "listening"
 * phase is actually two reception phases separated by a "short sleep" time. See
 * DW1000 User Manual section "Low-Power
 * Listening" for more details.
 *
 * NOTE: Before enabling low-power listening, the following functions have to be
 * called to fully configure it:
 *           - dwt_configuresleep() to configure long sleep phase. "mode"
 * parameter should at least have DWT_PRESRV_SLEEP,
 *             DWT_CONFIG and DWT_RX_EN set and "wake" parameter should at least
 * have both DWT_WAKE_SLPCNT and DWT_SLP_EN set.
 *           - dwt_calibratesleepcnt() and dwt_configuresleepcnt() to define the
 * "long sleep" phase duration.
 *           - dwt_setsnoozetime() to define the "short sleep" phase duration.
 *           - dwt_setpreambledetecttimeout() to define the reception phases
 * duration.
 *           - dwt_setinterrupt() to activate RX good frame interrupt
 * (DWT_INT_RFCG) only.
 *       When configured, low-power listening mode can be triggered either by
 * putting the DW1000 to sleep (using
 *       dwt_entersleep()) or by activating reception (using dwt_rxenable()).
 *
 *       Please refer to the low-power listening examples (examples 8a/8b
 * accompanying the API distribution on Decawave's
 *       website). They form a working example code that shows how to use
 * low-power listening correctly.
 *
 * input parameters:
 ...
 * no return value
 */
void dw1000_api::dwt_setlowpowerlistening(int enable) {
 ...
 *
 * input parameters:
 * @param snooze_time - "short sleep" phase duration, expressed in multiples of
 * 512/19.2 �s (~26.7 �s). The counter
 *                      automatically adds 1 to the value set. The smallest
 * working value that should be set is 1,
 ...
 * no return value
 */
void dw1000_api::dwt_setsnoozetime(uint8 snooze_time) {
 ...
 * DWT_START_RX_DELAYED        1 used to set up delayed RX, if "late" error
 * triggers, then the RX will be enabled immediately
 * (DWT_START_RX_DELAYED | DWT_IDLE_ON_DLY_ERR) 3 used to disable re-enabling of
 * receiver if delayed RX failed due to "late" error
 * (DWT_START_RX_IMMEDIATE | DWT_NO_SYNC_PTRS) 4 used to re-enable RX without
 * trying to sync IC and host side buffer pointers, typically when
 *                                               performing manual RX
 * re-enabling in double buffering mode
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error (e.g. a delayed
 * receive enable will be too far in the future if delayed time has passed)
 */
int dw1000_api::dwt_rxenable(int mode) {
    uint16 temp;
    uint8 temp1;
    ...
    if (mode & DWT_START_RX_DELAYED)  // check for errors
    {
        ...
        if ((temp1 & (SYS_STATUS_HPDWARN >> 24)) !=
            0)  // if delay has passed do immediate RX on unless
                // DWT_IDLE_ON_DLY_ERR is true
        {
 ...
 * no return value
 */
void dw1000_api::dwt_setrxtimeout(uint16 time) {
    uint8 temp;

    temp = dwt_read8bitoffsetreg(
        SYS_CFG_ID, 3);  // Read at offset 3 to get the upper byte only

    if (time > 0) {
        dwt_write16bitoffsetreg(RX_FWTO_ID, RX_FWTO_OFFSET, time);

        temp |=
            (uint8)(SYS_CFG_RXWTOE >>
                    24);  // Shift RXWTOE mask as we read the upper byte only
        // OR in 32bit value (1 bit set), I know this is in high byte.
        dw1000local.sysCFGreg |= SYS_CFG_RXWTOE;

        dwt_write8bitoffsetreg(
            SYS_CFG_ID, 3,
            temp);  // Write at offset 3 to write the upper byte only
    } else {
        temp &=
            ~((uint8)(SYS_CFG_RXWTOE >>
                      24));  // Shift RXWTOE mask as we read the upper byte only
        // AND in inverted 32bit value (1 bit clear), I know this is in high
        // byte.
        dw1000local.sysCFGreg &= ~(SYS_CFG_RXWTOE);
 ...
 * no return value
 */
void dw1000_api::dwt_setpreambledetecttimeout(uint16 timeout) {
 ...
 * no return value
 */
void dw1000_api::dwt_setinterrupt(uint32 bitmask, uint8 enable) {
    decaIrqStatus_t stat;
    uint32 mask;
 ...
 * no return value
 */
void dw1000_api::dwt_readeventcounters(dwt_deviceentcnts_t* counters) {
    uint32 temp;
 ...
 * no return value
 */
void dw1000_api::dwt_rxreset(void) {
    // Set RX reset
 ...
 * no return value
 */
void dw1000_api::dwt_softreset(void) {
 ...
 * no return value
 */
void dw1000_api::dwt_setxtaltrim(uint8 value) {
 ...
 */
uint8 dw1000_api::dwt_getinitxtaltrim(void) { return dw1000local.init_xtrim; }
 ...
 * no return value
 */
void dw1000_api::dwt_configcwmode(uint8 chan) {
#ifdef DWT_API_ERROR_CHECK
    assert((chan >= 1) && (chan <= 7) && (chan != 6));
#endif
 ...
 * no return value
 */
void dw1000_api::dwt_configcontinuousframemode(uint32 framerepetitionrate) {
    //
    ...
    dwt_write8bitoffsetreg(DIG_DIAG_ID, DIAG_TMC_OFFSET,
                           (uint8)(DIAG_TMC_TX_PSTM));  // Turn the tx power
                                                        // spectrum test mode -
 ...
 */
uint16 dw1000_api::dwt_readtempvbat(uint8 fastSPI) {
    uint8 wr_buf[2];
    uint8 vbat_raw;
    uint8 temp_raw;
    ...
    if (fastSPI == 1) {
        deca_sleep(1);  // If using PLL clocks(and fast SPI rate) then this
                        // sleep is needed
    ...
    } else  // change to a slow clock
    {
        _dwt_enableclocks(FORCE_SYS_XTI);  // NOTE: set system clock to XTI -
                                           // this is necessary to make sure the
                                           // values read are reliable
        // Read voltage and temperature.
        dwt_readfromdevice(TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, 2, wr_buf);
        // Default clocks (ENABLE_ALL_SEQ)
        _dwt_enableclocks(ENABLE_ALL_SEQ);  // Enable clocks for sequencing
 ...
 */
uint8 dw1000_api::dwt_readwakeuptemp(void) {
 ...
 */
uint8 dw1000_api::dwt_readwakeupvbat(void) {

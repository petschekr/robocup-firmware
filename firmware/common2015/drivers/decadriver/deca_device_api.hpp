...
#define DWT_PAC8 \
    0  //!< PAC  8 (recommended for RX of preamble length  128 and below
...
#define DWT_PAC64 \
    3  //!< PAC 64 (recommended for RX of preamble length 1024 and up
...
#define DWT_START_RX_DELAYED \
    1  // Set up delayed RX, if "late" error triggers, then the RX will be
       // enabled immediately
#define DWT_IDLE_ON_DLY_ERR \
    2  // If delayed RX failed due to "late" error then if this
// flag is set the RX will not be re-enabled immediately, and device will be in
// IDLE when function exits
#define DWT_NO_SYNC_PTRS \
    4  // Do not try to sync IC side and Host side buffer pointers when enabling
       // RX. This is used to perform manual RX
...
#define DWT_FF_COORD_EN \
    0x002  // behave as coordinator (can receive frames with no dest address
           // (PAN ID has to match))
...
#define DWT_INT_ARFE \
    0x20000000  // frame rejected (due to frame filtering configuration)

// DW1000 SLEEP and WAKEUP configuration parameters
#define DWT_PRESRV_SLEEP 0x0100  // PRES_SLEEP - on wakeup preserve sleep bit
#define DWT_LOADOPSET \
    0x0080  // ONW_L64P - on wakeup load operating parameter set for 64 PSR
#define DWT_CONFIG \
    0x0040  // ONW_LDC - on wakeup restore (load) the saved configurations (from
            // AON array into HIF)
#define DWT_RX_EN 0x0002  // ONW_RX - on wakeup activate reception
#define DWT_TANDV \
    0x0001  // ONW_RADC - on wakeup run ADC to sample temperature and voltage
            // sensor values
...
/*
 * From version 4.0.0:
 *  - dwt_setGPIOforEXTTRX: Replaced by dwt_setlnapamode to get equivalent
 * functionality.
 *  - dwt_setGPIOdirection: Renamed to dwt_setgpiodirection.
 *  - dwt_setGPIOvalue: Renamed to dwt_setgpiovalue.
 *  - dwt_setrxmode: Replaced by dwt_setsniffmode and dwt_setlowpowerlistening
 * depending on the RX mode the user
 *    wants to set up.
 *  - dwt_checkoverrun: As automatic RX re-enabling is not supported anymore,
 * this functions has become useless.
 *  - dwt_setautorxreenable: As automatic RX re-enabling is not supported
 * anymore, this functions has become
 *    useless.
 *  - dwt_getrangebias: Range bias correction values are platform dependent and
 * should therefore be managed at user
 *    application level.
 *  - dwt_xtaltrim: Renamed to dwt_setxtaltrim.
 *  - dwt_checkIRQ: Renamed to dwt_checkirq.
 *
 * From version 3.0.0:
 *  - dwt_getldotune: As LDO loading is now automatically managed by the driver,
 * this function has become useless.
 *  - dwt_getotptxpower: TX power values and location in OTP memory are platform
 * dependent and should therefore be
 *    managed at user application level.
 *  - dwt_readantennadelay: Antenna delay values and location in OTP memory are
 * platform dependent and should
...
public:
    dw1000_api();
    // Decawave(std::shared_ptr<SharedSPI> sharedSPI, PinName nCs, PinName
    // intPin);
protected:
    typedef void (dw1000_api::*dwt_cb_t)(const dwt_cb_data_t*);
    ...
    typedef struct {
        ...
        uint16 sleep_mode;  // Used for automatic reloading of LDO tune and
                            // microcode at wake-up
     ...
     *
     * NOTE: dwt_initialise() must be called prior to this function so that it
     * can return a relevant value.
     *
     * input parameters
     *
     * output parameters
     ...
     * @fn dwt_readdevid()
     *
     * @brief This is used to return the read device type and revision
     * information of the DW1000 device (MP part is 0xDECA0130)
     *
     * input parameters
     *
     * output parameters
     ...
     *
     * @brief This function enables/disables the fine grain TX sequencing
     * (enabled by default).
     *
     * input parameters
     ...
     * @fn dwt_setlnapamode()
     *
     * @brief This is used to enable GPIO for external LNA or PA functionality -
     * HW dependent, consult the DW1000 User Manual.
     *        This can also be used for debug as enabling TX and RX GPIOs is
     * quite handy to monitor DW1000's activity.
     *
     ...
     *
     * input parameters
     * @param gpioNum    -   this is the GPIO to configure - see GxM0... GxM8 in
     * the deca_regs.h file
     * @param direction  -   this sets the GPIO direction - see GxP0... GxP8 in
     * the deca_regs.h file
     *
     * output parameters
     *
     ...
     * @fn dwt_setgpiovalue()
     *
     * @brief This is used to set GPIO value as (1) or (0) only applies if the
     ...
     * supported
     * by this software (e.g. DW1000 32-bit device ID value is 0xDECA0130).
     * Then it
     ...
     *
     * input parameters
     * @param config    -   pointer to the configuration structure, which
     * contains the device configuration data.
     *
     * output parameters
     *
     * no return value
     ...
     * spectrum
     * including the power and pulse generator delay. The input is a pointer to
     * the data structure
     ...
     *
     * input parameters
     * @param enable - this enables or disables the TX smart power (1 = enable,
     * 0 = disable)
     *
     * output parameters
     *
     ...
     * TX buffer.  The input parameters are the data length in bytes and a
     * pointer
     * to those data bytes.
     *
     * input parameters
     ...
     * byte CRC.
     *                         Note: this is the length of TX message (including
     * the 2 byte CRC) - max is 1023
     *                         standard PHR mode allows up to 127 bytes
     *                         if > 127 is programmed, DWT_PHRMODE_EXT needs to
     * be set in the phrMode configuration
     *                         see dwt_configure function
     * @param txFrameBytes   - Pointer to the user�s buffer containing the data
     * to send.
     * @param txBufferOffset - This specifies an offset in the DW1000�s TX
     * Buffer at which to start writing data.
     *
     * output parameters
     *
     * returns DWT_SUCCESS for success, or DWT_ERROR for error
     ...
     * @fn dwt_writetxfctrl()
     *
     * @brief This API function configures the TX frame control register before
     * the transmission of a frame
     *
     * input parameters:
     * @param txFrameLength - this is the length of TX message (including the 2
     * byte CRC) - max is 1023
     *                              NOTE: standard PHR mode allows up to 127
     * bytes
     *                              if > 127 is programmed, DWT_PHRMODE_EXT
     * needs to be set in the phrMode configuration
     *                              see dwt_configure function
     * @param txBufferOffset - the offset in the tx buffer to start writing the
     * data
     ...
     * no return value
     */
    void dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset,
                          int ranging);
     ...
     * @fn dwt_starttx()
     *
     * @brief This call initiates the transmission, input parameter indicates
     * which TX mode is used see below
     *
     * input parameters:
     * @param mode - if 0 immediate TX (no response expected)
     *               if 1 delayed TX (no response expected)
     *               if 2 immediate TX (response expected - so the receiver will
     * be automatically turned on after TX is done)
     *               if 3 delayed TX (response expected - so the receiver will
     * be automatically turned on after TX is done)
     *
     * output parameters
     *
     ...
     *
     * @brief This API function configures the delayed transmit time or the
     * delayed RX on time
     *
     * input parameters
     * @param starttime - the TX/RX start time (the 32 bits should be the high
     * 32 bits of the system time at which to send the message,
     ...
     *
     * @brief This is used to read the TX timestamp (adjusted with the
     * programmed antenna delay)
     *
     * input parameters
     ...
     *
     * @brief This is used to read the high 32-bits of the TX timestamp
     * (adjusted with the programmed antenna delay)
     *
     * input parameters
     *
     * output parameters
     ...
     * "late" error the receiver will only be turned on if the
     * DWT_IDLE_ON_DLY_ERR is not set.
     * The receiver will stay turned on, listening to any messages until
     * it either receives a good frame, an error (CRC, PHY header, Reed Solomon)
     ...
     * (DWT_START_RX_IMMEDIATE | DWT_NO_SYNC_PTRS) 4 used to re-enable RX
     * without trying to sync IC and host side buffer pointers, typically when
     ...
     *
     * SNIFF mode is a low-power reception mode where the receiver is sequenced
     * on and off instead of being on all the time.
     * The time spent in each state (on/off) is specified through the parameters
     * below.
     * See DW1000 User Manual section 4.5 "Low-Power SNIFF mode" for more
     * details.
     *
     * input parameters:
     * @param enable - 1 to enable SNIFF mode, 0 to disable. When 0, all other
     * parameters are not taken into account.
     * @param timeOn - duration of receiver ON phase, expressed in multiples of
     * PAC size. The counter automatically adds 1 PAC
     *                 size to the value set. Min value that can be set is 1
     * (i.e. an ON time of 2 PAC size), max value is 15.
     * @param timeOff - duration of receiver OFF phase, expressed in multiples
     * of 128/125 �s (~1 �s). Max value is 255.
     *
     * output parameters
     *
     * no return value
     ...
     *
     * Low-power listening is a feature whereby the DW1000 is predominantly in
     * the SLEEP state but wakes periodically, (after
     * this "long sleep"), for a very short time to sample the air for a
     * preamble sequence. This preamble sampling "listening"
     * phase is actually two reception phases separated by a "short sleep" time.
     * See DW1000 User Manual section "Low-Power
     * Listening" for more details.
     *
     * NOTE: Before enabling low-power listening, the following functions have
     * to be called to fully configure it:
     *           - dwt_configuresleep() to configure long sleep phase. "mode"
     * parameter should at least have DWT_PRESRV_SLEEP,
     *             DWT_CONFIG and DWT_RX_EN set and "wake" parameter should at
     * least have both DWT_WAKE_SLPCNT and DWT_SLP_EN set.
     *           - dwt_calibratesleepcnt() and dwt_configuresleepcnt() to define
     * the "long sleep" phase duration.
     *           - dwt_setsnoozetime() to define the "short sleep" phase
     * duration.
     *           - dwt_setpreambledetecttimeout() to define the reception phases
     * duration.
     *           - dwt_setinterrupt() to activate RX good frame interrupt
     * (DWT_INT_RFCG) only.
     *       When configured, low-power listening mode can be triggered either
     * by putting the DW1000 to sleep (using
     *       dwt_entersleep()) or by activating reception (using
     * dwt_rxenable()).
     *
     ...
     *
     * @brief Set duration of "short sleep" phase when in low-power listening
     * mode.
     *
     * input parameters:
     * @param snooze_time - "short sleep" phase duration, expressed in multiples
     * of 512/19.2 �s (~26.7 �s). The counter
     ...
     *
     * input parameters
     * @param  timeout - Preamble detection timeout, expressed in multiples of
     * PAC size. The counter automatically adds 1 PAC
     *                   size to the value set. Min value that can be set is 1
     * (i.e. a timeout of 2 PAC size).
     *
     * output parameters
     *
     * no return value
     ...
     *
     * @brief calibrates the local oscillator as its frequency can vary between
     * 7 and 13kHz depending on temp and voltage
     *
     * NOTE: this function needs to be run before dwt_configuresleepcnt, so that
     * we know what the counter units are
     *
     * input parameters
     *
     * output parameters
     *
     * returns the number of XTAL/2 cycles per low-power oscillator cycle. LP
     * OSC frequency = 19.2 MHz/return value
     */
     ...
     *
     * @brief sets the sleep counter to new value, this function programs the
     * high 16-bits of the 28-bit counter
     *
     * NOTE: this function needs to be run before dwt_configuresleep, also the
     ...
     *
     * @brief configures the device for both DEEP_SLEEP and SLEEP modes, and
     * on-wake mode
     * i.e. before entering the sleep, the device should be programmed for TX or
     * RX, then upon "waking up" the TX/RX settings
     * will be preserved and the device can immediately perform the desired
     * action TX/RX
     *
     * NOTE: e.g. Tag operation - after deep sleep, the device needs to just
     * load the TX buffer and send the frame
     *
     *
     ...
     * transmission the device will enter deep sleep mode. The
     ...
     * the device will wake up on chip select line going low if the line is held
     * low for at least 500us.
     ...
     *
     * where fastest byte_rate is spi_rate (Hz) / 8 if the SPI is sending the
     * bytes back-to-back.
     * To save time and power, a system designer could determine byte_rate value
     * more precisely.
     *
     * NOTE: Alternatively the device can be waken up with WAKE_UP pin if
     * configured for that operation
     *
     * input parameters
     * @param buff   - this is a pointer to the dummy buffer which will be used
     * in the SPI read transaction used for the WAKE UP of the device
     ...
     * @fn dwt_setcallbacks()
     *
     * @brief This function is used to register the different callbacks called
     * when one of the corresponding event occurs.
     *
     * NOTE: Callbacks can be undefined (set to NULL). In this case, dwt_isr()
     * will process the event as usual but the 'null'
     * callback will not be called.
     *
     * input parameters
     * @param cbTxDone - the pointer to the TX confirmation event callback
     * function
     ...
     * @fn dwt_checkirq()
     *
     * @brief This function checks if the IRQ line is active - this is used
     * instead of interrupt handler
     *
     * input parameters
     *
     * output parameters
     ...
     *          - RXPHE/RXFCE/RXRFSL/RXSFDTO/AFFREJ/LDEERR (through cbRxTo
     cbRxErr)
     *        For all events, corresponding interrupts are cleared and necessary
     resets are performed. In addition, in the RXFCG case,
     *        received frame information and frame control are read before
     calling the callback. If double buffering is activated, it
     *        will also toggle between reception buffers once the reception
     callback processing has ended.
     *
     *        /!\ This version of the ISR supports double buffering but does not
     support automatic RX re-enabling!
     *
     * NOTE:  In PC based system using (Cheetah or ARM) USB to SPI converter
     there can be no interrupts, however we still need something
     *        to take the place of it and operate in a polled way. In an
     embedded system this function should be configured to be triggered
     ...
     * @fn dwt_isr_lplisten()
     *
     * @brief This is the DW1000's Interrupt Service Routine to use when
     * low-power listening scheme is implemented. It will
     ...
     *
     * @brief This function enables the specified events to trigger an
     * interrupt.
     ...
     * @param array - this is the pointer to the array into which to read the
     * data
     * @param length - this is the number of 32 bit words to read (array needs
     ...
     *
     * @brief This is used to enable the frame filtering - (the default option
     * is to
     * accept any data and ACK frames with correct destination address
     *
     * input parameters
     * @param - bitmask - enables/disables the frame filtering options according
     * to
     ...
     * otherwise it will be sent with a programmed delay (in symbols), max is
     * 255.
     * NOTE: needs to have frame filtering enabled as well
     *
     * input parameters
     * @param responseDelayTime - if non-zero the ACK is sent after this delay,
     * max is 255.
     *
     * output parameters
     *
     ...
     *
     * @brief This sets the receiver turn on delay time after a transmission of
     * a frame
     *
     * input parameters
     ...
     * @param rxBufferOffset - the offset in the rx buffer from which to read
     * the data
     *
     * output parameters
     *
     ...
     *
     * NOTE: Because of an internal memory access delay when reading the
     * accumulator the first octet output is a dummy octet
     ...
     * @param buffer - the buffer into which the data will be read
     * @param length - the length of data to read (in bytes)
     * @param accOffset - the offset in the acc buffer from which to read the
     * data
     *
     * output parameters
     *
     ...
     *
     * @brief This is used to select which Operational Parameter Set table to
     * load from OTP memory
     *
     * input parameters
     * @param ops_sel - Operational Parameter Set table to load:
     *                  DWT_OPSET_64LEN = 0x0 - load the operational parameter
     * set table for 64 length preamble configuration
     *                  DWT_OPSET_TIGHT = 0x1 - load the operational parameter
     * set table for tight xtal offsets (<1ppm)
     ...
     * @param address - this is the 16-bit OTP address into which the 32-bit
     * value is programmed
     *
     * output parameters
     *
     ...
     * @fn dwt_setleds()
     *
     * @brief This is used to set up Tx/Rx GPIOs which could be used to control
     * LEDs
     ...
     *          - bit 1: 1 to make LEDs blink once on init. Only valid if bit 0
     * is set (enable LEDs)
     *          - bit 2 to 7: reserved
     *
     * output parameters none
     ...
     *
     * input parameters:
     * @param   value - crystal trim value (in range 0x0 to 0x1F) 31 steps
     * (~1.5ppm per step)
     *
     * output parameters
     *
     ...
     *
     * NOTE: The value returned by this function is the initial value only! It
     * is not updated on dwt_setxtaltrim calls.
     *
     * input parameters
     *
     * output parameters
     ...
     *
     * input parameters:
     * @param chan - specifies the operating channel (e.g. 1, 2, 3, 4, 5, 6 or
     * 7)
     *
     * output parameters
     *
    ...
    * regulatory approvals testing.
     *
     * input parameters:
     * @param framerepetitionrate - This is a 32-bit value that is used to set
    * the interval between transmissions.
    *  The minimum value is 4. The units are approximately 8 ns. (or more
    * precisely 512/(499.2e6*128) seconds)).
     *
     * output parameters
     *
     * no return value
     ...
     * converters.
     * Note on Temperature: the temperature value needs to be converted to give
     ...
     * clock
     * however that means that the receiver will be switched off, if receiver
     * needs to be on then
     ...
     * on waking from Sleep/Deepsleep. They are not current values, but read on
     * last
     ...
     *
     * @brief this function reads the battery voltage of the DW1000 that was
     * sampled
     * on waking from Sleep/Deepsleep. They are not current values, but read on
     * last
     * wakeup if DWT_TANDV bit is set in mode parameter of dwt_configuresleep
     ...
     *        a. check if sub index is used, if subindexing is used - set bit-6
     * to 1 to signify that the sub-index address follows the register index
     * byte
     ...
     *
     *        2. Write the header followed by the data bytes to the DW1000
     * device
     *
     *
     * input parameters:
     * @param recordNumber  - ID of register file or buffer being accessed
     * @param index         - byte index into register file or buffer being
     * accessed
     * @param length        - number of bytes being written
     * @param buffer        - pointer to buffer containing the 'length' bytes to
     * be written
     *
     * output parameters
     *
    ...
    void dwt_writetodevice(
        ...
        const uint8* buffer  // input parameter - pointer to buffer containing
                             // the 'length' bytes to be written
        );
     ...
     * @param buffer        - pointer to buffer in which to return the read
     * data.
     *
     * output parameters
     *
    ...
    void dwt_readfromdevice(
        ...
        uint8* buffer  // input parameter - pointer to buffer in which to return
                       // the read data.
        );
...
#define dwt_read32bitreg(x) dwt_read32bitoffsetreg(x, 0)
     ...
     *
     * @brief  this function is used to read an 8-bit value from the DW1000
     * device registers
     *
     * input parameters:
     * @param regFileID - ID of register file or buffer being accessed
     ...
     *
     * @brief  this function is used to write an 8-bit value to the DW1000
     * device registers
     *
     * input parameters:
     * @param regFileID - ID of register file or buffer being accessed
     ...
     *
     * @brief
     * NB: In porting this to a particular microprocessor, the implementer needs
     * to define the two low
     * level abstract functions to write to and read from the SPI the
     * definitions should be in deca_spi.c file.
     * Low level abstract function to write to the SPI
     * Takes two separate byte buffers for write header and write data
     * returns 0 for success, or -1 for error
     *
     * Note: The body of this function is defined in deca_spi.c and is platform
     * specific
     *
     * input parameters:
     * @param headerLength  - number of bytes header being written
     * @param headerBuffer  - pointer to buffer containing the 'headerLength'
     * bytes of header to be written
     * @param bodylength    - number of bytes data being written
     * @param bodyBuffer    - pointer to buffer containing the 'bodylength'
    ...
    virtual int writetospi(uint16 headerLength, const uint8* headerBuffer,
                           uint32 bodylength, const uint8* bodyBuffer) = 0;
     ...
     * @param headerBuffer  - pointer to buffer containing the 'headerLength'
     * bytes of header to write
     * @param readlength    - number of bytes data being read
     * @param readBuffer    - pointer to buffer containing to return the data
     * (NB: size required = headerLength + readlength)
     *
     * output parameters
     *
     * returns DWT_SUCCESS for success (and the position in the buffer at which
     * data begins), or DWT_ERROR for error
     */
    virtual int readfromspi(uint16 headerLength, const uint8* headerBuffer,
                            uint32 readlength, uint8* readBuffer) = 0;
    ...
    //     processing may interact.  The code using this is kept to a minimum
    //     and the disabling time is also
    //     kept to a minimum, so blanket interrupt disable may be the easiest
    //     way to provide this.  But at a
    ...
    //     to map these calls transparently to the target system.  Alternatively
    //     the appropriate code may
    //     be embedded in the functions provided in the deca_irq.c file.
    //
    // ---------------------------------------------------------------------------

    // TODO typedef int decaIrqStatus_t ; // Type for remembering IRQ status

    /*!
     * ------------------------------------------------------------------------------------------------------------------
     * @fn decamutexon()
     *
     * @brief This function should disable interrupts. This is called at the
     * start of a critical section
     * It returns the IRQ state before disable, this value is used to re-enable
     * in decamutexoff call
     *
     * Note: The body of this function is defined in deca_mutex.c and is
     * platform specific
     *
     * input parameters:
     *
     ...
     */
    virtual decaIrqStatus_t decamutexon(void) = 0;
     ...
     * returns the state of the DW1000 interrupt
     */
    virtual void decamutexoff(decaIrqStatus_t s){};
     ...
     * no return value
     */
    virtual void deca_sleep(unsigned int time_ms){};
...
private:
    ...
    dwt_local_data_t dw1000local;  // Static local device data
};
